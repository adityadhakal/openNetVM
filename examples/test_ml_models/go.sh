...
while getopts ":p:n:f:m:t:g:" opt; do
  case $opt in
    p) print="-p $OPTARG";;
    n) instance="-n $OPTARG";;
    f) file_name="-f $OPTARG";;
    m) model_name="-m $OPTARG";;
    t) num_tries="-t $OPTARG";;
    g) gpu_percent="-g $OPTARG";;
    \?) echo "Unknown option -$OPTARG" && usage
    ;;
  esac
done
echo $SCRIPTPATH/build/app/bridge -l $cpu -n 3 --proc-type=secondary -- -r $service $instance -- $print $file_name $model_name $num_tries $gpu_percent
exec sudo $SCRIPTPATH/build/app/bridge -l $cpu -n 3 --proc-type=secondary -- -r $service $instance -- $print $file_name $model_name $num_tries $gpu_percent
