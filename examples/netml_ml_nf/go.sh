...
while getopts ":p:n:f:m:b:" opt; do
  case $opt in
    p) print="-p $OPTARG";;
    n) instance="-n $OPTARG";;
...
exec sudo $SCRIPTPATH/build/app/bridge -l $cpu -n 3 --proc-type=secondary -- -r $service $instance -- $print $file_name $model_name $batch_size $gpu_percent
