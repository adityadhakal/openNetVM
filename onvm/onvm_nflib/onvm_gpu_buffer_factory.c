...
void give_cpu_addresses(uint8_t batch_id, void ** input_buffer, void ** output_buffer) {
	...
	if(cpu_buffer_state[batch_id].occupancy_indicator < MAX_IMAGES_PER_PARTITION ) {
		...
		*input_buffer = cpu_buffer_state[batch_id].cpu_buffers[buffer_state[batch_id].occupancy_indicator];
		*output_buffer = cpu_buffer_state[batch_id].output_cpu_buffers[buffer_state[batch_id].occupancy_indicator++];
	}
	else {
